--------------------------------------------------------------------
-- |
-- Module   : Text.Regex.PCRE.Light.Text
...
--
-- * 'no_utf8_check'   - Do not check the pattern for UTF-8 validity (always enabled in this module)
--
...
--
compile :: Text -> [PCREOption] -> Regex
compile str os = S.compile (encodeUtf8 str) (no_utf8_check:utf8:os)
{-# INLINE compile #-}

-- | 'compileM'
-- A safe version of 'compile' with failure lifted into an Either
compileM :: Text -> [PCREOption] -> Either String Regex
compileM str os = S.compileM (encodeUtf8 str) (no_utf8_check:utf8:os)
{-# INLINE compileM #-}
...
match r subject os =
    case S.match r (encodeUtf8 subject) os of
           Nothing -> Nothing
           Just x  -> Just (map decodeUtf8 x)
{-# INLINE match #-}
